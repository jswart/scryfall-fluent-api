...
public class CardsCodeNumberTest {
  ...
  public void runtTests() {
    ...
    // no options
    try {
      url = Query.cards().codeNumberLang().build();
    ...
    // missing options
    try {
      url = Query.cards().codeNumberLang().withSet(SetDBEnum.AetherRevolt).build();
      assert (false); // set must be defined
    } catch (final Exception e) {
    }

    // minimum required options
    url = Query.cards().codeNumberLang().withSet(SetDBEnum.AetherRevolt).withCollectorNumber(5).build();
    assertEquals("https://api.scryfall.com/cards/aer/5/en", url);
    url = Query.cards().codeNumberLang().withSet(SetDBEnum.AetherRevolt).withCollectorNumber(5).withLang().english()
        .build();
    assertEquals("https://api.scryfall.com/cards/aer/5/en", url);
    ...
    // default
    url = Query.cards().codeNumberLang().withSet(SetDBEnum.AetherRevolt).withCollectorNumber(5).withVersion().large()
        .build();
    assertEquals("https://api.scryfall.com/cards/aer/5/en", url);
    // non-default
    url = Query.cards().codeNumberLang().withSet(SetDBEnum.AetherRevolt).withCollectorNumber(5).withVersion().small()
        .build();
    assertEquals("https://api.scryfall.com/cards/aer/5/en?version=small", url);
    url = Query.cards().codeNumberLang().withSet(SetDBEnum.AetherRevolt).withCollectorNumber(5).withVersion().normal()
        .build();
